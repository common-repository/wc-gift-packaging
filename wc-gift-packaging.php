<?php
	...
	*/
	...
	function wc_gift_packaging_fee() { 
		...
		global $woocommerce, $gift_packaging_fee;
	 	...
	 	if( $_POST ):
			...
			if( ( isset( $data[ 'wc-gift-packaging' ] ) OR isset( $_POST[ 'wc-gift-packaging' ] ) ) AND !empty( $gift_packaging_fee ) ):
	...
	function wc_gift_packaging_settings() {
	...
	add_filter( 'woocommerce_general_settings', 'gift_pack_admin_settings' );

	function gift_pack_admin_settings( $settings ) {

		
		$settings[] = array(
			'name' => __( 'WC Gift Packaging', 'woocommerce' ),
			'type' => 'title',
			'desc' => __( 'The following options affect how the WC Gift Packaging plugin works.', 'woocommerce' ),
			'id'   => 'wc_gift_packaging_settings'
		);
